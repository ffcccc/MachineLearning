...
# class DecisionTree(AClassifier.aClassifier):
...
class RegressionNode(dt.DecisionNode):
    pass
# def __init__(self, index=-1, value=None, results=None, right_tree=None, left_tree=None):
...
class RegressionTree:
    ...
    def createRegressionTree(self, data):
        ...
        if len(data) == 0:
            self.tree_node = RegressionNode(results=self.getMean(data[:, -1]))
            return self.tree_node

        # sample_num, 
        feature_dim = np.shape(data)
    ...
    def predict(self, test_data):
        ...
        prediction = np.zeros([test_num, 1])
        for i in range(test_num):
            prediction[i] = dt.DecisionTreeClassifier.classify(test_data[i, :], self.tree_node)
        self.prediction = prediction
    ...
    def pruning(self, tree, val_data):
        if len(val_data) == 0:
            self.tree_node = RegressionNode(results=(tree.left_tree.result + tree.right_tree.result)/2)
            return self.tree_node
